...
const Pagination = ({pageNumber, paginate, currentpage, prev , next, filtercate}) => {
  ...
  <ul className="inline-flex -space-x-px text-sm">
    ...
    <li onClick={()=>paginate(item)}>
      <a  className={` ${ currentpage == i + 1 ? "flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-[#262626] border border-gray-300" : "flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-[#fff] border border-gray-300"}`}>{item +1}</a>
    </li>
    ))}
